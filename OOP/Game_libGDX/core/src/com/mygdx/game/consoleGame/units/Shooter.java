...
public abstract class Shooter extends Character {
     ...
     */
    public Shooter(int maxHealth, int attackDamage, int defence, Texture texture, int x, int y, double minAccuracy, int maxArrows) {
        super(maxHealth, attackDamage, defence, 3, texture, x, y);
    ...
    public void step(ArrayList<Character> targetTeam, ArrayList<Character> friendTeam) {
        ...
        for (Character friend : friendTeam) {
            if (friend instanceof Peasant) {
                if (friend.state.equals(States.READY)) {
                    arrows++;
